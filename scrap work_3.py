...
all_lin_reg_simple.linear_formula()
...
all_lin_reg_multi.split_data()
all_lin_reg_multi.full_linear_formula()
...
#########backwards elimination for linear regression
...
linear_params_summary(['price','distance', 'temperature','precipIntensity', 'humidity', 'windSpeed', 'time', 'weekday'], ['Uber', 'Lyft'], ['Standard'], 0.55)
...
#    return y_train
#    X = df_filter.loc[:, df_filter.columns == 'distance']
##    X = df_filter[variables]    
#    Y = df_filter['price']
##    X = sm.add_constant(X)
#    X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size = 0.2, random_state = 0)
...
#    X_train = X_train['distance'].values
#    y_train = y_train.values
    weights = np.polyfit(X_train, y_train, degree)
    model = np.poly1d(weights)
#    results = smf.ols(formula = 'y_train ~model(X_train)', data = combined_df).fit()
...
reg_poly_RMSE(['distance', 'price'],['Uber', 'Lyft'], ['Standard'],3)
...
def performance_metrics(cab_type, ride_type):
    ...
    simple_df = pd.DataFrame(np.asarray(reg_simple_list).reshape(1,5), columns = ['Regression Type','r2', 'MAE', 'MSE', 'RMSE'])
    ...
    multi_df = pd.DataFrame(np.asarray(reg_multi_list).reshape(1,5), columns = ['Regression Type','r2', 'MAE', 'MSE', 'RMSE'])
    ...
    quad_df = pd.DataFrame(np.asarray(reg_quad_list).reshape(1,5), columns = ['Regression Type','r2', 'MAE', 'MSE', 'RMSE'])
    ...
    poly_df = pd.DataFrame(np.asarray(reg_poly_list).reshape(1,5), columns = ['Regression Type','r2', 'MAE', 'MSE', 'RMSE'])

    all_dfs = [simple_df, multi_df, quad_df, poly_df]
    all_results_df = pd.concat(all_dfs).reset_index(drop = True)
    return all_results_df

test = performance_metrics(['Uber', 'Lyft'], ['Standard'])
...
all_lin_reg_simple = MyKnnRegression(['price','distance'], ['Uber', 'Lyft'],['Standard'], 3)
all_lin_reg_simple.metric_RMSE()
